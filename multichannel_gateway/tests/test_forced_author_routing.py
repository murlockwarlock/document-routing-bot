...
class TestEditorReportsForFixedRoute(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_no_reply_fixed_editor_reports_use_original_message_and_resolved_reply(self):
        task = telegram_file_info(
            "forced",
            42,
            "курсовая работа.docx",
        ...
        handlers, nik1, _nik2 = await self._make_handlers_with_tasks([task], sent, settings)
        normal_path = Path(tempfile.gettempdir()) / "no-reply-normal.pdf"
        ai_path = Path(tempfile.gettempdir()) / "no-reply-ai.pdf"
        normal_path.write_bytes(b"normal")
        ai_path.write_bytes(b"ai")

        class ClientBoundMessage:
            def __init__(self, file_name, message_id, result_path):
                self.id = message_id
                self.reply_to_message_id = None
                self.document = SimpleNamespace(file_name=file_name)
                self.text = None
                self.chat = SimpleNamespace(id=301)
                self.from_user = SimpleNamespace(username="editor", id=9000)
                self._client = object()
            ...
            def __copy__(self):
                ...
                clone.__dict__ = {
                    ...
                    if key not in {"_client", "download_objects"}
                }
                clone.download_objects = []
                return clone

            async def download(self, path):
                self.download_objects.append(self)
                if not hasattr(self, "_client"):
                    raise AttributeError("'Message' object has no attribute '_client'")
                Path(path).write_bytes(Path(self.result_path).read_bytes())
                return path

        normal = ClientBoundMessage("курсовая работа.pdf", 4201, normal_path)
        ai = ClientBoundMessage("ИИ курсовая работа.pdf", 4202, ai_path)
        client = SimpleNamespace(name="НИК-2")

        with patch("bot_handlers.Config.get_setting", side_effect=settings_lookup(settings)):
            await handlers.handle_main_account(client, normal)
            await handlers.handle_main_account(client, ai)

        self.assertEqual([normal], normal.download_objects)
        self.assertEqual([ai], ai.download_objects)
        self.assertEqual(0, normal.copy_attempts)
        self.assertEqual(0, ai.copy_attempts)
    ...
    async def test_download_failure_releases_incoming_message_guard_for_retry(self):
        task = telegram_file_info(
            "forced",
            42,
            "курсовая работа.docx",
        ...
        sent = [SimpleNamespace(id=300330, chat=SimpleNamespace(id=301))]
        handlers, nik1, _nik2 = await self._make_handlers_with_tasks([task], sent, settings)
        result_path = Path(tempfile.gettempdir()) / "retry-after-download-failure.pdf"
        result_path.write_bytes(b"retry")
        message = await self.make_response("курсовая работа.pdf", 300330, 301, result_path, message_id=4301)
        attempts = 0
        ...
        with patch("bot_handlers.Config.get_setting", side_effect=settings_lookup(settings)), patch("builtins.print"):
            await handlers.handle_editor_response(client, message)
            self.assertNotIn(("301", "4301"), handlers._editor_response_guard)
            await handlers.handle_editor_response(client, message)

        self.assertEqual(2, attempts)
        self.assertEqual(1, nik1.send_document.await_count)
        self.assertIn(("301", "4301"), handlers._editor_response_guard)

    async def test_successful_incoming_editor_message_is_delivered_only_once(self):
        task = telegram_file_info(
            "forced",
            42,
            "курсовая работа.docx",
        ...
        sent = [SimpleNamespace(id=300330, chat=SimpleNamespace(id=301))]
        handlers, nik1, _nik2 = await self._make_handlers_with_tasks([task], sent, settings)
        result_path = Path(tempfile.gettempdir()) / "duplicate-incoming.pdf"
        result_path.write_bytes(b"duplicate")
        message = await self.make_response("курсовая работа.pdf", 300330, 301, result_path, message_id=4302)
        downloads = []
        ...
        self.assertEqual([message], downloads)
        self.assertEqual(1, nik1.send_document.await_count)
